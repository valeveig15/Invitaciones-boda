(() => {
  ...
  // MUSIC: clean, single implementation.
  ...
  function closeWelcome(){
    ...
    setTimeout(() => welcomeMusic.remove(),600);
  }

  enterWithMusic?.addEventListener('click', () => {
    if(!audio){ closeWelcome(); return; }

    audio.volume = .45;
    audio.currentTime = 0;

    // play() happens immediately inside the user click handler.
    const promise = audio.play();
    if(promise && typeof promise.then === 'function'){
      promise.then(() => {
        musicFab?.classList.remove('has-error');
        updateMusicButton();
        closeWelcome();
      }).catch(err => {
        console.error('Audio failed:',err);
        musicFab?.classList.add('has-error');
        enterWithMusic.textContent = 'INTENTAR DE NUEVO';
      });
    }else{
      updateMusicButton();
      closeWelcome();
  ...
  enterWithoutMusic?.addEventListener('click',closeWelcome);
  ...
  audio?.addEventListener('error',() => musicFab?.classList.add('has-error'));
  audio?.load();
  updateMusicButton();
